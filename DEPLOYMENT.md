...
在 Aster 中确认每个子账户是**全仓、单资产 USD1、双向持仓模式**。程序读取并检查这些设置，不自动修改保证金或持仓模式。已有仓位必须多空数量一致、没有挂单；账户内有逐仓持仓时会等待处理。
...
保存后执行 `sudo aster-desk restart`。登录网页，添加账户，模式选“实盘”，环境变量前缀填 `ASTER_A` 或 `ASTER_B`。账户默认暂停；读取成功后可检查实际持仓、杠杆、USD1 余额和保证金比率，再点击启动。修改服务器密钥需要重启服务。

每个账户可以设置额度阈值和每批每边的金额上限（默认 10,000 / 1,000 USD1）。最终仓位由风险、余额和额度决定；单批金额只是拆单上限，不是总持仓目标。模拟账户采用独立账本，不向交易所下单，也不发送飞书通知。

## 交易口径与执行

- 同时监控 4x、5x、10x、20x，以及账户实际杠杆对应的公开额度。空仓首次只在 4x 额度严格超过阈值时开始。
- 每批多空等数量，盘口价差 `(ask-bid)/mid ≤ 0.0005`。以 BBO 限价 FOK 批量提交，每轮完成后重新读取账户；实际周期包含请求耗时和至少 5 秒等待。
- 保证金比率使用 USD1 资产的 `maintMargin / (crossWalletBalance + crossUnPnl)`，与单资产全仓界面口径对应，不使用账户顶层 USDT 汇总字段。
- 每笔同时考虑两边手续费、价差及标记价格偏差、维持保证金档位、可用余额、最小数量和盘口深度。预测成交后的比率必须**严格小于 50%**；接近上限时自动缩小每批数量，无法容纳最小一笔时等待。每次成交后再检查实际比率；达到上限时暂停新加仓。
- 有持仓时逐级检查下一个配置档位：目标杠杆公开剩余额度严格大于多空总持仓名义价值，且账户档位容纳现有总持仓，才调整杠杆。确认调整成功后，继续使用**新的实际杠杆额度**判断和计算后续加仓。已有非标准杠杆从下一个更高配置档位继续。
...
价格、手续费和维持保证金会变化，预测约束不等于任何时刻实际比率都能保持低于 50%。程序达到上限时暂停加仓，不会自动平掉整个现有组合。

## 飞书、升级与运维

完成一轮连续加仓后，暂停、达到风险限制，或连续 60 秒不能继续加仓时，汇总发送账户、成交批次、每个标的与杠杆、总名义金额及实际保证金比率。通知队列持久化，失败重试；飞书已收到但响应丢失时可能重复。没有配置 Webhook 时保留待发消息，网页显示待发送数量。
