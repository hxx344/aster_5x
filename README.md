...
同时监控 **XAUUSD1、SPCXUSD1、CLUSD1**，每个标的按 **5x、每 5 秒检查、严格大于 10,000 USD1** 的规则独立触发。默认关闭飞书，触发只记入本地日志。无需第三方 Python 依赖。

三个标的分别保存提醒状态和冷却时间，一个标的的持续达标不会压制其他标的的提醒；普通网络失败只让对应标的退避，其他标的继续检查。遇到 429/418 限流则暂停新的查询请求，遵守服务端退避时间。
...
**升级：重新执行安装命令，配置与提醒历史会保留，默认自动加入 SPCXUSD1 和 CLUSD1。** 原有 XAUUSD1 提醒状态会自动迁移，阈值、间隔和飞书设置继续沿用。如果手动设置了 `symbols` 列表，则只监控该列表；支持上述三个代码。无交互安装可用 `curl -fsSL https://raw.githubusercontent.com/hxx344/aster_5x/main/install.sh | sudo bash -s -- --non-interactive`，之后通过 `sudo aster-5x configure` 配置。已克隆仓库时也可执行 `sudo bash install.sh`，直接安装当前目录的代码。

程序位于 `/opt/aster-5x`，配置位于 `/etc/aster-5x/config.json`，状态和轮转日志位于 `/var/lib/aster-5x`。服务以独立 `aster-5x` 用户运行。`status` 分别显示三个标的的额度、检查时间和错误；只有所有标的数据新鲜且成功时才返回成功。部分成功显示 `partial`，不把单个成功当成全部正常。首次查询失败时安装命令会返回失败并显示原因，但已安装的服务仍会按退避规则重试；用 `logs` 排查。查看 systemd 原始状态：`sudo systemctl status aster-5x`。
...
停止：`python monitor.py --stop`。再次启动用 `start.ps1`。配置修改后需要停止并重启。`runtime/status.json` 的 `markets` 按交易代码保存检查时间、当前值与运行状态；顶层保存 PID、标的列表、阈值和推送开关。只有对应标的 `status=ok` 且检查时间新鲜时才表示最新查询成功。错误状态保留上次成功值和时间，不能当成当前值。
...
每个标的首次检查已达标会提醒；持续达标只提醒一次；回落至阈值及以下后重新触发，同一标的两次提醒至少间隔 300 秒。冷却期间发生的新触发只在冷却结束后仍达标时发送。成功记录持久化，重启避免重复；发送失败会重试。飞书消息包含对应交易代码和交易页面链接。飞书已接收但响应丢失或发送后进程崩溃时，重试仍可能产生重复消息。
