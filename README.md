...
循环支持每账户的**每日成交量上限**（USD1，0 为不限）：按实际成交时间的 UTC 日期累计多空开仓、平仓和补救成交，不含手续费；每笔成交显示当日累计量。开仓前预留预计平仓量，额度不足时缩小下一轮或等待，UTC 00:00 后自动重新检查开仓条件。手动暂停不会自动恢复；已有仓位的平仓和补救继续允许，实际价格变动可能使最终成交量超过预留。
