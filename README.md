...
新增 Linux 网页账户管理：多账户双向市价分批开仓，仅监控 **5x / 10x / 20x**。每账户可设置最低开仓杠杆（**5x / 10x / 20x**，默认 **5x**）和风险约束上限（大于 0%、不超过 100%，默认 **50%**），暂停策略后在网页“策略设置”修改。BBO 价差不超过万 5。每仓占用保证金为 `|数量| × 标记价格 ÷ 实际杠杆`，全部仓位多空分别累加后除以 USD1 账户总权益，预测成交后的占用率不超过该账户设置。升杠杆后按新杠杆重算占用和额度并继续加仓，完成后汇总飞书通知。实盘凭据只放服务器环境变量。完整安装、配置和执行规则见 [Linux 交易部署说明](DEPLOYMENT.md)。

```bash
curl -fsSL https://raw.githubusercontent.com/hxx344/aster_5x/main/install-trading.sh | sudo bash
```

网页每个账户新增独立的**多空循环**：可分别设置固定杠杆（含 2x）、品种、深度价差采样金额、bp 阈值、开仓名义金额范围和持仓时长。默认示例为 **XAUUSD1 / 2x / 每边 1 万 USD1 深度价差 ≤ 0.1 bp / 每边开仓不超过 1 万 USD1 / 持仓 60 秒**。两边开仓核实后开始计时，时间到达且价差再次达标才提交双向平仓，核实空仓后进入下一轮。保存不会启动交易；暂停账户后可配置，启动按钮运行所选模式。同账户不能同时运行自动加仓、仓位迁移与循环；不同账户可独立选择。配置、计时和订单核对进度随升级保留。规则及异常处理见 [多空循环说明](DEPLOYMENT.md#独立多空循环)。
...
循环支持每账户的**每日成交量上限**（USD1，0 为不限），同时限制 **UTC 自然日**和**滚动 24 小时**的成交量：累计多空开仓、平仓和补救成交，不含手续费；每笔成交显示当日累计量。开仓按两个窗口较小的剩余额度预留预计平仓量，额度不足时缩小下一轮或等待。UTC 00:00 只重置自然日统计，最近 24 小时成交仍计入额度，直到逐笔满 24 小时移出窗口；两个窗口及交易条件均满足后才自动恢复。手动暂停不会自动恢复；已有仓位的平仓和补救继续允许，实际价格变动可能使最终成交量超过预留。

每日及近 24 小时同时显示**循环交易成本**：按指定 taker 费率 **0.0125%** 计算全部实际成交的手续费，加上同批等量买卖的实际成交差价。每笔明细拆分手续费、该笔计入的差价和总成本；有利差价可抵减成本。未配齐或仍在同步时标记成本未完整，历史已入账成交自动补算。此处为指定口径的成本统计，不含资金费或返佣。

循环条件不满足时，面板的**上次检查结果**显示实际值、要求值及检查时间：包括采样/委托价差、双边深度、交易所最小委托、可用余额、全账户保证金占比和两个成交量窗口。最小一轮无法执行时，列出能确定的失败条件与资金缺口；数据不足的项目显示未核验。日志也保留具体失败数值，页面刷新不会更新旧检查的时间。

循环选择的品种在**本账户禁止普通 5x / 10x / 20x 加仓**，公共额度满足也不例外。普通下单入口会再次读取该账户保存的循环设置，页面保留真实额度并显示禁用原因；其他账户的同品种不受此限制。暂停账户或等待日/24 小时额度不会解除循环品种保护，关闭循环模式后解除。原有同账户策略互斥规则继续适用。

新版 `aster-desk` 的额度监控独立于盘口刷新，各标的三档共用一次采样，完成后等待 **2 秒**再查。10x / 20x 额度满足已启用账户的网页阈值并通过持仓容量初筛，可用状态发生变化时，立即优先唤醒升杠杆及开仓路径，跳过普通账户轮次间隔；优先从 10x、20x 中选择可用的更高档，确认升档后立即尝试首批开仓。账户状态、额度、盘口和风控仍会重新检查，接口退避、预算及开仓冷却仍生效；请求耗时会增加延迟，不保证即时成交。
