...
class NotificationBackoffTests(unittest.TestCase):
    def setUp(self):
        self.config = {
            "symbols": ["XAUUSD1", "SPCXUSD1"], "leverages": [4, 5],
    ...
    def test_feishu_limit_preserves_tier_retry_without_global_market_backoff(self):
        for retry in (180, 86400):
            with self.subTest(retry=retry):
                ...
                with patch("monitor.sample", return_value={4: self.reading(15000), 5: self.reading(0)}), \
                ...
                self.assertEqual(markets["XAUUSD1:4"]["retry_seconds"], retry)
                self.assertEqual(tracker.next_due["XAUUSD1:4"], self.now + retry)
                self.assertFalse(records["XAUUSD1:4"]["gate"]["notified"])
                self.assertEqual(markets["XAUUSD1:5"]["status"], "ok")
    ...
    def test_other_market_and_tier_continue_while_throttled_delivery_waits(self):
        ...
        with patch("monitor.sample", return_value={4: self.reading(15000), 5: self.reading(0)}) as sample, \
            ...
            self.assertEqual(set(markets), {"XAUUSD1:5"})
            self.assertEqual(global_retry, 0)
            self.assertEqual(other.check()[0]["SPCXUSD1:4"]["status"], "ok")
