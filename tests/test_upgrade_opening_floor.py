...
class UpgradeOpeningFloorTests(unittest.TestCase):
    def setUp(self):
        ...
        self.f.account['policy'].update(symbols=list(SYMBOLS), min_open_leverage=10, margin_limit='0.9')
    ...
    def test_cl_upgrades_below_floor_then_opens_only_after_reaching_floor(self):
        ...
        with patch.object(self.f.broker, 'set_leverage', wraps=self.f.broker.set_leverage) as change, \
            ...
            self.engine.tick_account('test')  # Confirm actual 5x.
            ...
            self.engine.tick_account('test')
            self.assertEqual(change.call_args.args, (CL, 10))
            self.engine.tick_account('test')  # Confirm actual 10x before entry.
            submit.assert_not_called()
            self.engine.tick_account('test')
            submit.assert_called_once()
            self.assertEqual({order['symbol'] for order in submit.call_args.args[0]}, {CL})

    def test_supported_tiers_below_twenty_remain_available_for_upgrades(self):
        self.f.account['policy']['min_open_leverage'] = 20
        self.f.store.save_account(self.f.account)
        with patch.object(self.f.broker, 'set_leverage', wraps=self.f.broker.set_leverage) as change, \
             patch.object(self.f.broker, 'submit', side_effect=AssertionError('must not open below 20x')):
            for tier in (5, 10):
    ...
    def test_restart_confirms_upgrade_without_opening_below_floor(self):
        ...
        self.assertEqual([p.leverage for p in self.f.broker.snapshot(SYMBOLS).pair(CL)], [5, 5])
